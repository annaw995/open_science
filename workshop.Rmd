...
matrix(myvec, ncol = 2, dimnames = list(c("A", "B", "D"), c("X", "Y"))) # creates a matrix with row and column names
```
...
### Data frames
Data frames also are two dimensional data containers. Unlike matrices each column can contain a different types of data. They are created using `data.frame()`. One can also convert into data frames by using the function `as.data.frame()`: 
...
```
Like matrices one could also name the rows: 
```{r}
...
So, lets create an exemplary function that calculates the mean, standard deviation and median of a series of numbers: 
...
- Always comment your code so you can still understand what you did a few years later.
...
summarize( # function to perform summarising functions on data
  group_by( # group data by some variables (sex and some_binary_characterisation)
    as.tibble( # make a tibble out of a data frame
      mydf # the data
      ),
...
# note that all of the functions used in this code (besides n() and mean()) are part of the tidyverse  
...
- train the model on several variations of the training set since every variation would change the model outcome. 
- do some sort of cross-validation (k-fold-cross-validations) while training. 
- use all above to select the optimal model parameters.
- test those model parameters on new data. 

For now, we will skip all steps beside the last one. But first let's have a look onto our model and how well it fits our data we trained it on. We can do that by using base R and the plot function. It will return four different plots: 
...
Predictions %>% 
  ...
  # group by each model 
  group_by(model_vars) %>% 
  # calculate different types of correlation between predicted and actual data
...
We already did a correlation test on all our models. We calculated Pearson and Spearman Correlations of our predictions and also looked onto the corresponding p-values. So, now we need to do Hypothesis Tests.
...
Another option would be, to perform multiple U-Tests comparing all the models amongst each other.
...
multi.test <- function(dist1, dist2, dt){
  ...
  dt %>% 
    summarise(p.value = wilcox.test(unlist(.[, dist1]), unlist(.[, dist2]), paired = FALSE)$p.value, 
              test.stat = wilcox.test(unlist(.[, dist1]), unlist(.[, dist2]), paired = FALSE)$statistic) -> test
...
lapply(Distributions, function(dist1){
  lapply(Distributions[which(Distributions != dist1)], function(dist2, dt){
    multi.test(dist1, dist2, dt) %>% 
      # add column so we later know which distributions were compared with each other
      add_column(comparison = paste(dist1, " vs ", dist2, sep = ""), 
                 # add another column to later filter for unique columns
                 filter = paste(sort(c(dist1, dist2)), collapse = "_")) -> test
    return(test)
  }, Predictions) %>% 
  ...
  # same
  bind_rows() %>% 
  # filter table for unique values in filter so we have each result once
...
Again, none of our tests is significant. Still, the correlation test clearly showed an increasing correlations between predicted probabilities and real probabilities as soon as the alcohol consumption was taken into account. Also, the correlation coefficients for the full model and the one only based on age and alcohol consumption were pretty high. Additionally, the ANOVA of our linear model including all features showed that alcohol consumption and age groups explained significantly more variance as the tobacco consumption.
Altogether it indicates that a linear model based on age groups and alcohol consumption already is sufficient to predict the probability of developing esophageal cancer. 
...
3. Keep your model as easy as possible, especially if you want to interpret it. 
