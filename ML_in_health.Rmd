...
## General
R ([download here](https://www.r-project.org/))^[R Core Team (2021). R: A language and environment for statistical computing. R
  Foundation for Statistical Computing, Vienna, Austria. URL https://www.R-project.org/.] is an open source environment for statistical programming which is mainly used for data analysis (including data cleaning and arranging, calculations, statistics, machine learning and plotting). It is running on all systems and can either be used in the console or via a graphical interface called R Studio ([download here](https://www.rstudio.com/products/rstudio/)). Feel free to download and install both and copy and paste the code blocks within this script into R/R Studio to follow each step.
...
To start with, there are a few differences between R and many other programming languages one should know:

- R starts counting at 1 and not 0.
- assigning variables is usually done by an arrow `->` in either direction (pointing to the variable name), the equal sign `=` is mainly used to define parameters in functions.
- R is case sensitive.
...
And calculate something using these variables: 
```{r}
(a+b)^2*a
```


You can also ask for logical responses, e.g. when comparing numerical values. Thereby, a `TRUE` can also be seen as a `1` and a `FALSE` as `0`.  Notice that "is equal to" is asked for using `==` and "is not equal to" using `!=`:
...
R can not only work with numerical (or logical) values but also with character values (also called strings): 
...
You can not do mathematical calculations on strings. Running e.g. `x+y` will return an error message. However, you can ask whether one string is equal or not to another and recieve a logical response:
```{r}
x != y 
x == y
```
...
You can also write your own functions (see chapter 1.4). 
...
You can select specific elements in this vector by specifying the index of this element within the vector. It is also possible to select several elements at once using a vector of indices. Indices of vectors (and all other data containers) are defined in squared brackets `[]`. Important note: Counting in R starts with 1, not with 0 as in many other programming languages. The first element in any data container has the index of 1. 
...
And then e.g. use the logical vector returned by e.g. `1 == myvec` to filter your data: 
```{r}
myvec[1==myvec] # filter myvec for all values that are 1
myvec[myvec > 2] # filter myvec for all values that are > 2
```

You can also ask for the indices where `myvec` is 1 using the following function: 
```{r}
which(myvec==1)
```

### Matrices
Matrices are two-dimensional data containers that can only contain data of one type. They are created out of vectors using the function `matrix()`. Other data types, e.g. data frames, can also be converted to matrices using the function `as.matrix()`.
...
Note that the selection of a single row/column will return a vector. 
...
### Data frames
Data frames also are two-dimensional data containers. Unlike in matrices each column can contain a different type of data. They are created out of vectors using `data.frame()`. One can also convert e.g. a matrix into data frames by using the function `as.data.frame()`: 
...
Just like matrices, rows can be named as follows:
```{r}
...
The functions `rownames()` and `colnames()` can be used to set the corresponding names in every data type that has rows and columns (including data types that are not from base R). Vectors and lists can also contain named elements. In this case you would use the function `names()`.
...
### Lists
Lists are literal containers that can store any object: single variables, Vectors, matrices, data frames, other lists, tibbles, ...
...
Elements in lists can be accessed using the index. Using single square bracktes like for vectors will return a sublist. To directly access an element in the list double square brackets `mylist[[index/name_of_list_element]]` or the `$` are used: 
```{r}
...
## Functions
Previously, we used several in-build functions. Since R is a mostly function-based language (not object-based), all tasks are done by calling a function and giving parameters to this function. Basic R comes with hundreds of functions to perform calculations, plotting or other tasks. Packages further expand the amount of functions to perform tasks easier. Often, functions called in R rely on other programming languages like C#, Python or Pearl. 

All in all, functions are wrappers to perform more complex actions within one line to minimize the amount of copy & paste errors and lines of code. Hence, it is useful to also know how to write your own functions. 

So, lets create an exemplary function that calculates the mean, standard deviation and median of a series of numbers and also introduce conditions in which the function will do one thing and not the other: 
...
There is much more to say and learn about R. However, the initial topic of our workshop is machine learning in health so the above part really was only meant to give you a small introduction into R and how it works in case you stumble over R again (if you stay in science you most likely will at some point). Also, it might be helpful to understand the things we did in the workshop and that are written in the following part. 

Finally, there are some general advises regarding any type of programming as well as R specific: 

- Don't be afraid of searching for functions / how to's on the internet. The R community is pretty helpful and many questions might have been already asked (Especially looking at [Stackoverflow](https://stackoverflow.com/) always is a good option). Also, most functions and packages are well documented.
...
Generally, R is able to read in all types of data that can be downloaded from anywhere. You can for example load in any types of tables (.csv, .tsv, .xlxs, .txt, ...), plain text files or even images. For the sake of simplicity you do not need to manually download data. (Most databases sadly are not very user friendly. So keep that in mind when you search for usable data.) 
Instead, we will use an [(o)esophageal cancer data set](https://stat.ethz.ch/R-manual/R-devel/library/datasets/html/esoph.html)^[Breslow, N. E. and Day, N. E. (1980) Statistical Methods in Cancer Research. Volume 1: The Analysis of Case-Control Studies. IARC Lyon / Oxford University Press.] that is accessible after installing R.
...
So, `esoph` is a data frame with 88 rows and 5 columns. But what data is contained in esoph?

And what data do we actually explore? What is esophageal cancer?

As mentioned previously, "esoph" is a (o)esophageal cancer data set. The esophagus is the muscle food pipe between your mouth and stomach. Esophageal cancer is a subgroup of cancers of the head and neck region that together have a global incidence of > 800,000 case. More than half of these cases arise in the esophagus.^[Bray, F. et al. Global cancer statistics 2018: GLOBOCAN estimates of incidence and mortality worldwide for 36 cancers in 185 countries. CA: A Cancer Journal for Clinicians 68, 394–424 (2018).] Head and neck cancers generally have a low survival rate that simply said depends on the risk factors one was exposed to and the region they arise. Esophageal cancers belong to the cancers with the most unfavorable survival chances (5-year survival rate of 22-24 % in Europe). Risk factors that increase the likelihood to develop esophageal cancer mainly are age, tobacco and alcohol consumption and weight.^[https://www.krebsdaten.de/Krebs/EN/Content/Cancer_sites/Oesophageal_cancer/oesophageal_cancer_node.html] 


The esoph data set we explore contains data from a case-control study of esophageal cancer conducted in Ile-et-Vilaine, France. It contains numbers of diagnosed cancers across different groups of risk factors. Let's have a closer look on them by checking the columnames of esoph: 
...
So, we have the number of cancers, the size of the control group and the risk factors age, alcohol and tobacco consumption. But how does the data actually look like? 
We can check that in several ways. We can have a look on the first few columns or ask R to give us some sort of overview or summary.  
```{r}
...
We now had a first glimpse on our data. However, to actually understand our data more than a glimpse is necessary. We could go through the whole data frame and read everything by just printing the whole table. However, the bigger the data the nastier it is to do so. Instead of looking on our table of data we can start to visualize it. For example we could simply look how many cases are detected in each age group:

```{r}
esoph %>% 
  # make a ggplot with number of cases on the x axis and the age groups on the y axis
  ggplot(aes(x = ncases, y = agegp)) +
  # specify you want adot plot 
  geom_point() 

```

**Exercise: What other plot types could you use to visualize the number of cases? Try them out! Also plot the number of cases for the missing variables!**



The number of cases within each group is nice to know but not really meaningful. Instead of looking at the absolute numbers we rather should look at the number of cancer diagnosis within all visited patients. For example, we could calculate the fraction of positive cases per age group, without taking alcohol or tobacco consumption into account: 
...
**Exercise: Reproduce this plot with the other grouping parameters! Try out different themes!**



Another option to visualize the data are heatmaps. Heatmaps can be used to visualize even high dimensional data (like mRNA expression levels of 20,000 genes measured in 1000 samples). This high dimensional data usually gets clustered in different ways (e.g. hierarchical clustering or k-means) so one can see patches of high or low expressed genes among the different samples. Since our data is already grouped and ordered we don't need to use a clustering algorithm and can just plot it: 
...
**Exercise: Plot the other grouping parameter combinations. Try out different coloring schemes.**


We could also visualize our whole data set at once: 
...
Now we can train linear models to predict the fraction. We will train our model on 2/3 of our data and use the remaining third as a test set of unknown data which our model aims to predict. Therefore, we will split the data set randomly. Note: Randomly splitting something means that every time it is repeated other data points will be selected which will slightly variate the model. However, computers can not be "random", each randomness always is based on some reference points. We can set these reference points and thus making our model reproducible by using a seed: 
...
To develop an actual Machine Learning algorithm you would not keep it that simple. You would:

- train the model on several variations of the training set since every variation would change the model outcome (k-fold-cross-validation). 
- do several iterations with different randomly selected training data sets.
- test those model parameters on new data (which is why we only selected 2/3 of our data for training). 
- use all above (especially the first two) to select the optimal model parameters.

To keep it simple and calculation times minimal we will only test our model on new data (the one not selected for training). But first, lets have a look on our model and how well it fits the data we trained it on. We can do that by using base R and the plot function. It will return four different plots: 
...
And then plot our predicted data vs our real data. Ideally we would expect a perfect diagonal where predicted data perfectly matches the real data. We will also calculate the correlation between predicted and real data. There are different types of correlations one can calculate. Generally, they return a statistic that is any number between -1 (anti-correlated) and 1 (correlated) with 0 meaning no correlation at all. Here, we will use Pearson and Spearman Correlation. Pearson Correlation uses the actual values while Spearman Correlation uses the ranks of our data points (checking whether the lowest predicted data point is also the lowest actual data point). 
...
Yes! Our models (all of them) are better then our random model. But which of them is most accurate? Do we need the full model or are age and alcohol consumption sufficient to explain everything? Remember: Keep the model as simple as possible! 
...
Based on our prediction accuracies we already got the impression that increasing age and alcohol consumption have the strongest impact on developing esophageal cancer. We can check that by looking at the model itself. First, let's look at the summary of our full model: 
...
Initially, we do a Kruskal-Wallis-Test:
...
1. Correlation is not causation! One of the most famous examples is the high correlation between high stork population and a high human baby birth rate.^[Matthews, Robert. (2000). Storks Deliver Babies (p= 0.008). Teaching Statistics. 22. 36 - 38. 10.1111/1467-9639.00013.] Always keep in mind what data you are working with and whether what your analysis tells you makes sense. 

2. Know how to interpret models (and take a look at meta data). Based on our given data set we could also argue that getting cancer increases the alcohol consumption (if we know that the alcohol consumption started years before the cancer was diagnosed this conclusion becomes nonsense). 
