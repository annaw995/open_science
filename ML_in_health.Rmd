...
R can not only work with numerical (or logical) values but also with character values (strings): 
...
You can not do calculations with mathematical operators on strings. Running e.g. `x+y` will return an error message. However, you can ask logical questions:
```{r}
x != y 
x == y
```
